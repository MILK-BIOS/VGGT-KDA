...
def main(args):
    if args.eval_dataset == "nyu":
        depth_pathes = glob.glob("/commondocument/fhs/datasets/NYUv2/val/nyu_depths/*.npy")
        ...
        if full:
            depth_pathes = glob.glob(f"/commondocument/fhs/datasets/Sintel/training/depth/*/*.dpt")
            depth_pathes = sorted(depth_pathes)
        else:
            seq_list = [
                "alley_2",
            ...
            depth_pathes_folder = [
                f"/commondocument/fhs/datasets/Sintel/training/depth/{seq}" for seq in seq_list
            ]
            depth_pathes = []
        ...
        img_pathes_folder = [
            f"/commondocument/fhs/datasets/Bonn/rgbd_bonn_{seq}/rgb_110/*.png"
            for seq in seq_list
        ]
        img_pathes = []
        ...
        depth_pathes_folder = [
            f"/commondocument/fhs/datasets/Bonn/rgbd_bonn_{seq}/depth_110/*.png"
            for seq in seq_list
        ]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
        ...
        depth_pathes = glob.glob(
            "/commondocument/fhs/datasets/KITTI/depth_selection/val_selection_cropped/groundtruth_depth_gathered/*/*.png"
        )
        depth_pathes = sorted(depth_pathes)
        pred_pathes = glob.glob(
